...
- 중복된 연락처 : `"이미 사용중인 번호에요. 번호를 다시 확인해 주세요."`
...
: 회원 인증 API 성공 시 return 된 `patientId` 와 `patientPhoneNumber` 를 담아서 요청합니다.
...
== 13. 사용자 마케팅 동의 여부 수정
*사용자 마케팅 동의 여부 수정 API*

*[ 개발 중 잘못된 Request나 데이터 오류로 인해 발생할 수 있는 경우 ]*::

- 사용자 권한이 아닌 경우 : `"권한이 없는 사용자입니다."`
- 사용자 조회 불가능 : `"존재하지 않는 사용자입니다."`
- 잘못된 서비스 수정인 경우 : `"존재하지 않는 서비스 이용 동의입니다."`
...
- 사용자 권한이 아닌 경우 : `"권한이 없는 사용자입니다."`
- 사용자 조회 불가능 : `"회원 정보를 조회할 수 없습니다."`
- 서비스 이용동의 조회 불가능 : `"존재하지 않는 서비스 이용 동의입니다."`
...
- 분석 상태가 'FAIL'인 경우 : `"양치 상태 체크 확인을 실패했습니다. 재촬영 바랍니다."`
- 분석 결과 저장 실패 : `"양치 상태 체크 확인 결과 데이터 저장에 실패했습니다."`

*[ 개발 중 잘못된 Request나 데이터 오류로 인해 발생할 수 있는 경우 ]*::

- 사용자 권한이 아닌 경우 : `"권한이 없는 사용자입니다."`
- 사용자 조회 불가능 : `"존재하지 않는 사용자입니다."`
- 사진 업로드 경로가 없는 경우 : `"파일 저장에 실패했습니다."`
- 분석 결과 값이 null인 경우 : `"양치 상태 체크 확인 결과 저장에 실패했습니다... 관리자에게 문의 바랍니다."`
- 인터럽트 에러 : `"Thread interrupted"`
...
- 사용자 권한이 아닌 경우 : `"권한이 없는 사용자입니다."`
- 사용자 조회 불가능 : `"존재하지 않는 사용자입니다."`
- 존재하지 않는 구강검진 고유 번호 : `"존재하지 않는 구강 검진입니다."`
- 회원 정보와 구강검진 정보 미일치 : `"회원 정보와 구강 검진 정보가 일치하지 않습니다."`
...
- 문진표 항목 누락 : `"~번 문항을 입력해 주세요."`
- 유효하지 않은 값 : `"~번 문항에 ~ 값은 유효하지 않습니다."`
- 중복 선택 시 동일 값 중복 : `"~번 문항에 ~ 값이 중복으로 존재합니다."`
...
*[ 개발 중 잘못된 Request나 데이터 오류로 인해 발생할 수 있는 경우 ]*::
- 존재하지 않는 콘텐츠의 경우 : `"존재하지 않는 콘텐츠입니다."`
...
*[ 개발 중 잘못된 Request나 데이터 오류로 인해 발생할 수 있는 경우 ]*::
- 존재하지 않는 약관의 경우 : `"존재하지 않는 약관입니다."`

=== Http
include::{snippets}/service-agreement/list/http-request.adoc[]
=== Request
include::{snippets}/service-agreement/list/query-parameters.adoc[]
=== Response
include::{snippets}/service-agreement/list/response-fields.adoc[]
include::{snippets}/service-agreement/list/response-body.adoc[]