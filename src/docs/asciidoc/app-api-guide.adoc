...
- 서비스 이용 필수 동의 '미동의' : `"필수 항목을 동의해주세요."` (rt : 400)
...
- 분석 상태가 'FAIL'인 경우 : `"양치 상태 체크 확인을 실패했습니다. 재촬영 바랍니다."` (rt : 410, 411)
- 분석 결과 저장 실패 : `"양치 상태 체크 확인 결과 데이터 저장에 실패했습니다."` (rt : 400)

*[ 개발 중 잘못된 Request나 데이터 오류로 인해 발생할 수 있는 경우 ]*::

- 사용자 권한이 아닌 경우 : `"권한이 없는 사용자입니다."` (rt : 401)
- 사용자 조회 불가능 : `"존재하지 않는 사용자입니다."` (rt : 416)
- 사진 업로드 경로가 없는 경우 : `"파일 저장에 실패했습니다."` (rt : 400)
- 분석 결과 값이 null인 경우 : `"양치 상태 체크 확인 결과 저장에 실패했습니다... 관리자에게 문의 바랍니다."` (rt : 400)
- 인터럽트 에러 : `"Thread interrupted"` (rt : 400)
